...
export const ResetEmailForm = () => {
  ...
  const resetRequest = (data: any) => {
    ...
    const payload = {
      email,
      redirectUrl: `${baseUrl}/reset-password/new`,
    };

    // @ts-ignore
    dispatch(forgotPasswordRequest(payload)).then((res: any) => {
  ...
  const renderSumModal = () => {
    ...
    if (isEmailSent) {
      return (
        <ResetEmailConfirmation email={email} resetRequest={resetRequest} />
      );
    }
  };
