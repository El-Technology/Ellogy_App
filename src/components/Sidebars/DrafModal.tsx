...
import {
  ...
  getTicketsByUserId,
  updateTicket,
} from "src/store/ticket-service/asyncActions";
...
export const DraftModal: FC<DraftModalProps> = ({
  handleCloseModal,
  ticket,
  userId,
  isCreating,
  ...
  const handleSave = () => {
    if (activeTicket) {
      if (!activeTicket.id) {
        dispatch(createTicket({ userId, ticket }))
          .then(() => dispatch(getTicketsByUserId({ userId })))
          .then((res: any) => {
